            ...
            switch (comando) {
                ...
                case "download":
                    ...
                    if(textInformationURLDownload.includes("mediafire.com/file/")) {
                        ...
                        enviar("Aguarde...")
                        await fetch(`https://tohka.tech/api/dl/igdl?link=${q}&apikey=KzqKxVmU65`).then(async(api) => api.json()).then(async(json) => {
                            if(json.status != "operando") return enviar("Link invalido ou a API está offline! Tente novamente...")
                            //await fetch(`${json.resultado.link[0].thumbnail_link}`).then((api1) => api1.buffer()).then((buffer) => {
                                //fs.writeFileSync(`./Logs/imageTempThumb.jpg`, buffer)
                                cooh.sendMessage(from, { video: { url: `${json.resultado.link[0].download_link}`} }, { quoted: info })
                ...
                case "ship":
                    ...
                    if (args[1] == '' || args[1] == undefined || !args[1]) {
                        ...
                        */
                        ...
                        if(numeroTel1.length == 10) numeroTel1Fix = `${sender.split("@")[0].slice(0, 4)}9${sender.split("@")[0].slice(4)}`
                        ...
                        const user2Avatar = await cooh.profilePictureUrl(`${numeroTel1Fix}@c.us`) == "item-not-found" ? "http://storage.cdworld.cloud/r/sem-imagem-avatar.png" : await cooh.profilePictureUrl(`${numeroTel1Fix}@c.us`)
